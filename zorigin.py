...
import gemeentefunc as g

# geonames username
global_username = "scriptie_vdwal2"
...
#    print(dataframe.values[0])
   # columnnames: articleID, toponym, geoID, isTitle, predID, fcodes, lookUp
    coopy = dataframe.copy()

    for index, row in dataframe[dataframe["predID"]==0].iterrows():
 #       str = ''
        if row.fcodes!=[]:   # url with codes, miss kan de if check wel weg
            for code in row.fcodes:
#                print(code)
                string = "&featureClass=" # + code
                string += code[0]
               # make URL and find ID (per fcode)
                ID = funID(row.lookUp, string)

#            print(coopy.at[index, "predID"])
        else:
            ID = 0
#            ID = findID_baseline(row.lookUp)

        coopy.at[index, "predID"] = ID  # ehh moet nog hierarchie maken
...
def readTSV(dataset, baseline):
    ...
    if baseline:    # perform the baseline program
        ...
        df = g.process_annotation(dataset, "dataset.csv")
...
#    processed_dataset = readTSV(df, args.baseline)
